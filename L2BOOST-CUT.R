...
L2BOOSTCUT <- function(dat_tr, dat_te, ntree, tau, mmax = 1e5, tol = 1e-5) {
  ...
  for (j in seq_len(n_train)) {
    ...
    while (v.index.tr < w.index.tr) {
      ...
      egy2p.tr[j] <- egy2p.tr[j] + (log(t[v.index.tr]))^2 * 
        (s.sm.tr[j, v.index.tr + 1] - s.sm.tr[j, v.index.tr])
      v.index.tr <- v.index.tr + 1
    }
    egy1.tr[j] <- egy1p.tr[j] / denom.tr[j]
    egy2.tr[j] <- egy2p.tr[j] / denom.tr[j]
  }
  ...
  for (j in seq_len(n_test)) {
    ...
    while (v.index.te < w.index.te) {
      ...
      egy2p.te[j] <- egy2p.te[j] + (log(t[v.index.te]))^2 * 
        (s.sm.te[j, v.index.te + 1] - s.sm.te[j, v.index.te])
      v.index.te <- v.index.te + 1
    }
    egy1.te[j] <- egy1p.te[j] / denom.te[j]
    egy2.te[j] <- egy2p.te[j] / denom.te[j]
  }
  ...
  for (j in seq_len(mmax)) {
    ...
    loss_tr <- append(loss_tr, cut_loss(egy2.tr, egy1.tr, f1_tr))
    loss_te <- append(loss_te, cut_loss(egy2.te, egy1.te, f1_te))
